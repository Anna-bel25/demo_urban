...
class VisitRegisterScreen extends StatelessWidget {
  ...
  Widget _formulario(VisitProvider model, BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (model.medioIngresoSeleccionado == 'Vehiculo') ...[
            CustomButtonGradientRedForms(
              text: 'Seleccionar Imagen de Placa',
              onPressed: () async {
                await _pickImage(model, context);
              },
              style: botonEstiloGradientRed(),
            ),
            if (model.fotoPlaca != null) ...[
              const SizedBox(height: 10),
  ...
  Widget _botones(VisitProvider model, BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          CustomButtonGradientBlack(
            ...
            onPressed: () => model.createSolicitudVisita(context),
  ...
  Future<void> _selectDateTime(BuildContext context, VisitProvider model) async {
  ...
  Future<void> _pickImage(VisitProvider model, BuildContext context) async {
    ...
    if (pickedFile != null) {
      ...
      model.setFotoPlaca(imageFile);
      //await model.setFotoPlaca(imageFile);

      showCenteredDialog(context, "Imagen de placa seleccionada.");
    } else {
      print("No se seleccionó ninguna imagen.");
      showCenteredDialog(context, "No se seleccionó ninguna imagen.");
    }
  }
